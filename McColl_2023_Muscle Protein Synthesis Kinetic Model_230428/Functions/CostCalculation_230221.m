function [Title, RMS_CostIndividual, RMS_CostTotal, RMS_norm_CostIndividual, RMS_norm_CostTotal, RSS_CostIndividual, RSS_CostTotal] = ...
    CostCalculation_230221(t, x, Akt_S_T, p70S6K_T, EqDur, CostParameters, ExpData, FSRsim)
...
%       Akt_S_T = position within data set
...
Akt_S_iv = x(t_endEq(1), Akt_S_T);
p70S6K_T_iv = x(t_endEq(1), p70S6K_T);
...
for n = 1:length(CostParameters)
    if ExpDataMatrix(n,1,1) == Akt_S_T
        rows = find(ExpData(:,1) == CostParameters(n));
        ExpDataMatrix(n, 1:length(rows),2) = ExpData(rows, 2) + EqDur; %Adjusted to Equilibrium period
        ExpDataMatrix(n, 1:length(rows),3) = ExpData(rows, 3) * Akt_S_iv;
        ExpDataMatrix(n, 1:length(rows),4) = sqrt(Akt_S_iv^2 * ExpData(rows, 4).^2); % Transforming SE data (factoring in IV)
            
    elseif ExpDataMatrix(n,1,1) == p70S6K_T
        rows = find(ExpData(:,1) == CostParameters(n));
        ExpDataMatrix(n, 1:length(rows),2) = ExpData(rows, 2) + EqDur; %Adjusted to Equilibrium period
