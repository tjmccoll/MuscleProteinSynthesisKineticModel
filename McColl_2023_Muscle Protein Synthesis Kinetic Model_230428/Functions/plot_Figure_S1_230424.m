function [CostIndividual, CostTotal, mps_integral_grams] = plot_Figure_S1_230424(figNum, t, x_conc, x_mass, ExpData, ...
...
subplotTight(height_y, length_x, 17:20)
    plot (t,F_ma, '-', 'LineWidth',1.25, 'Color',[0 0 1]) % F_m,a
    grid on
        ax=gca;
        ax.FontSize = fontSize_units;
    xlim(x_lim)
    ...
    xtickangle(Angle)
hold on
    plot (t,(F_m0), '-', 'LineWidth',1.25, 'Color',[1 0 0]) % F_m,0
hold on 
...
lines = p70S6KT389_pos;
    ...
    xticklabels(xTicksSeq)
        ...
        if Protocol == "Valid., Glynn (2010) 1.85g"
            ylim_max = 0.21e-7; %* Glynn et al.: 1.85 g validation dataset ylim value
            ylim([0 ylim_max])
        elseif Protocol == "Valid., Dickinson (2011) cntl"
            ylim_max = 0.24e-7; %* Dickinson et al.: validation ylim value
            ylim([0 ylim_max])
        else
            ylim([0 p70S6K_total(1,1)*1.1]) % ylim for all other validation datasets
        end
    ylabel('')
    xlabel('')
