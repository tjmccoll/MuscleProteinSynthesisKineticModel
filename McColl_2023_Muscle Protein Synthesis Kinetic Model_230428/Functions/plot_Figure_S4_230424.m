function [CostIndividual, CostTotal, mps_integral_grams] = plot_Figure_S4_230424(figNum, t, x_conc, x_mass, ExpData, ...
    CostParameters, RunDuration, EquilibriumDuration, FilePath, FileName, kValues, interventionDuration)
% Plotting function that creates Supplementary Figure S4. The plot function
% simulates leucine, plasma insulin, p70S6K, and protein balance with
% associated experimental data and costs. 
%   [CostIndividual, CostTotal, mps_integral_grams] = 
%       plot_Figure_S4_230221(figNum, t, x_conc, x_mass, ExpData,
...
if any(CostIndividual(:,1)==LeucinePlasma_pos) && any(CostIndividual(:,1)==LeucineInt_pos) % text if Fm,a and Fm,0 exist
    ...
    text(0.99*(EquilibriumDuration+simDuration), 0.85*1.4e-3, textOutput, ...
...
elseif any(CostIndividual(:,1)==LeucinePlasma_pos)
    ...
    text(0.99*(EquilibriumDuration+simDuration), 0.63*1.4e-3, textOutput, ...
        'VerticalAlignment', 'top', 'HorizontalAlignment', 'right',...
        'FontSize', fontSize_inPlotText)
end
hold off
legend({'Plasma', 'Intracellular'}, 'NumColumns',2);
...
text(t(mps_max_t)*0.85, 3*mps_max, textOutput, ...
