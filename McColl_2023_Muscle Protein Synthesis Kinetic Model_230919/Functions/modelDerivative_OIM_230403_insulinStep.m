function [dx] = modelDerivative_OIM_230403_insulinStep(t, x, KValueVector_given, OptimizeParameters, ...
    OptimizeParameters_initial, KValueVector_factor, EquilibriumDuration, InsulinStepDur, GlucoseInfRate, ...
...
% Leucine infusion rate maintained throughout simulation
LeucineInfRate = LeucineInfRate_given;


dx = zeros(40,1);

% insulin concentration step function
if (t < EquilibriumDuration)
    dx(3) = f1 - E*(InsulinPlasma/V_p - InsulinRemote/V_i) - InsulinPlasma/t_p + r4;
elseif (t >= EquilibriumDuration) && (t < EquilibriumDuration+InsulinStepDur) 
    dx(3) = 0;
elseif (t>= EquilibriumDuration+InsulinStepDur)
    dx(3) = f1 - E*(InsulinPlasma/V_p - InsulinRemote/V_i) - InsulinPlasma/t_p + r4;
end
