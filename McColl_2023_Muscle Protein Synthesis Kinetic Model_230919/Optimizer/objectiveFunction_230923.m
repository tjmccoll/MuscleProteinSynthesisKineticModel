function [CostTotal] = objectiveFunction_230923(KValueVector_given, ...
    KValueVector_factor, InitialValues_mass, OptimizeParameters, OptimizeParameters_initial, ExpData, ...
    CostParameters, AminoAcidInput, EquilibriumDuration, runDuration, interventionDuration, GlucoseInfRate, ...
    LeucineInfRate, plasmaVolume, skeletalMuscleVolume)
...
%% 3-pool parameters   
        ...
        x_bolus(:,43) = KValue_opt(6)*x_bolus(:,4) - ...
            KValue_opt(7)*x_bolus(:,6); % Net Balance (incorrect)
...
FSR_integral = mps_integral/LeucineMolarMass;
...
x_conc_moleMin(:,41) = x(:,41); % F_ma in units of mole/min
x_conc_moleMin(:,42) = x(:,42); % F_m0 in units of mole/min
x_conc_moleMin(:,43) = nan; % NB: not correct calculation
x_conc_moleMin(:,44) = x(:,44); % MPS in units of mole/min
x_conc_moleMin(:,45) = x(:,45); % MPB in units of mole/min
...
[Title, ~, ~, CostIndividual, CostTotal] = CostCalculation_230920(t, x_conc_moleMin, Akt_pS_pos, p70S6KT389_pos, ...
    EquilibriumDuration, CostParameters, ExpData, FSR_integral);
